...
{
    CHECK_ANY(ltoir_output_path != nullptr);
    CHECK_ANY(symbol_prefix != nullptr);
    CHECK_ANY(include_dirs == nullptr);
    CHECK_ANY(mathdx_include_dir == nullptr);
    CHECK_ANY(num_include_dirs == 0);
    ...
    // Same descriptor setup as wp_cuda_compile_dot, but with TENSORS API
    CHECK_CUBLASDX(cublasdxSetOperatorInt64(
        h, cublasdxOperatorType::CUBLASDX_OPERATOR_FUNCTION, cublasdxFunction::CUBLASDX_FUNCTION_MM
    ));
    CHECK_CUBLASDX(cublasdxSetOperatorInt64(
    ...
    CHECK_CUBLASDX(cublasdxSetOperatorInt64(
        h, cublasdxOperatorType::CUBLASDX_OPERATOR_API, cublasdxApi::CUBLASDX_API_TENSORS
    ));
    std::array<long long int, 3> precisions = { precision_A, precision_B, precision_C };
    CHECK_CUBLASDX(
        cublasdxSetOperatorInt64s(h, cublasdxOperatorType::CUBLASDX_OPERATOR_PRECISION, 3, precisions.data())
    ...
    // TENSOR API: arrangement must be 3 elements (A, B, C) like the standard API
    std::array<long long int, 3> arrangement = { arrangement_A, arrangement_B, 0 }; // C defaults to col-major
    CHECK_CUBLASDX(cublasdxSetOperatorInt64s(
        h, cublasdxOperatorType::CUBLASDX_OPERATOR_ARRANGEMENT, arrangement.size(), arrangement.data()
    ));

    // Create tensors:
    // - SMEM_A/B: plain shared memory with user's arrangement (for tile_load data)
    // - SUGGESTED_SMEM_A/B: cuBLASDx-optimized layout (input to execute)
    // - SUGGESTED_RMEM_C: register accumulator
    // - SUGGESTED_SMEM_C: shared memory copy target for RMEM
    cublasdxTensor tensor_a_plain, tensor_b_plain;
    cublasdxTensor tensor_a_sugg, tensor_b_sugg, tensor_c_rmem, tensor_c_smem;
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SMEM_A, &tensor_a_plain));
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SMEM_B, &tensor_b_plain));
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SUGGESTED_SMEM_A, &tensor_a_sugg));
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SUGGESTED_SMEM_B, &tensor_b_sugg));
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SUGGESTED_RMEM_C, &tensor_c_rmem));
    CHECK_CUBLASDX(cublasdxCreateTensor(h, cublasdxTensorType::CUBLASDX_TENSOR_SUGGESTED_SMEM_C, &tensor_c_smem));

    // Finalize all tensors
    std::array<cublasdxTensor, 6> tensors = { tensor_a_plain, tensor_b_plain, tensor_a_sugg, tensor_b_sugg, tensor_c_rmem, tensor_c_smem };
    CHECK_CUBLASDX(cublasdxFinalizeTensors(tensors.size(), tensors.data()));

    // Query traits — use max of plain and suggested for shared memory allocation
    long long int val, val2;
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_c_rmem, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val));
    *rmem_storage_bytes = (int)val;
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_c_rmem, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_LOGICAL_SIZE, &val));
    *rmem_logical_size = (int)val;
    // For A: plain is what tile_load allocates, suggested is temp buffer for COPY
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_a_plain, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val));
    *smem_a_storage_bytes = (int)val;
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_a_sugg, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val2));
    *smem_a_sugg_storage_bytes = (int)val2;
    // For B: same
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_b_plain, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val));
    *smem_b_storage_bytes = (int)val;
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_b_sugg, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val2));
    *smem_b_sugg_storage_bytes = (int)val2;
    CHECK_CUBLASDX(cublasdxGetTensorTraitInt64(tensor_c_smem, cublasdxTensorTrait::CUBLASDX_TENSOR_TRAIT_STORAGE_BYTES, &val));
    *smem_c_storage_bytes = (int)val;

    // Create device functions
    // COPY_A: rearrange A from plain SMEM layout to suggested SMEM layout
    cublasdxDeviceFunction df_copy_a;
    std::array<cublasdxTensor, 2> copy_a_tensors = { tensor_a_plain, tensor_a_sugg };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_COPY,
        copy_a_tensors.size(), copy_a_tensors.data(), &df_copy_a
    ));

    // COPY_B: rearrange B from plain SMEM layout to suggested SMEM layout
    cublasdxDeviceFunction df_copy_b;
    std::array<cublasdxTensor, 2> copy_b_tensors = { tensor_b_plain, tensor_b_sugg };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_COPY,
        copy_b_tensors.size(), copy_b_tensors.data(), &df_copy_b
    ));

    // EXECUTE: C_rmem += A_sugg @ B_sugg (no alpha/beta for RMEM)
    cublasdxDeviceFunction df_execute;
    std::array<cublasdxTensor, 3> execute_tensors = { tensor_a_sugg, tensor_b_sugg, tensor_c_rmem };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_EXECUTE,
        execute_tensors.size(), execute_tensors.data(), &df_execute
    ));

    // CLEAR: zero-init C_rmem
    cublasdxDeviceFunction df_clear;
    std::array<cublasdxTensor, 1> clear_tensors = { tensor_c_rmem };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_CLEAR,
        clear_tensors.size(), clear_tensors.data(), &df_clear
    ));

    // COPY_C: C_rmem -> C_smem
    cublasdxDeviceFunction df_copy;
    std::array<cublasdxTensor, 2> copy_tensors = { tensor_c_rmem, tensor_c_smem };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_COPY,
        copy_tensors.size(), copy_tensors.data(), &df_copy
    ));

    // MAP_IDX2CRD: iterate over C_rmem elements
    cublasdxDeviceFunction df_map;
    std::array<cublasdxTensor, 1> map_tensors = { tensor_c_rmem };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_MAP_IDX2CRD,
        map_tensors.size(), map_tensors.data(), &df_map
    ));

    // IS_INDEX_IN_BOUNDS: check if index valid for C_rmem
    cublasdxDeviceFunction df_bounds;
    std::array<cublasdxTensor, 1> bounds_tensors = { tensor_c_rmem };
    CHECK_CUBLASDX(cublasdxCreateDeviceFunction(
        h, cublasdxDeviceFunctionType::CUBLASDX_DEVICE_FUNCTION_IS_INDEX_IN_BOUNDS,
    ...
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_copy_a, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_copy_b", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_copy_b, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_execute", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_execute, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_clear", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_clear, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_copy", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_copy, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_map", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
        df_map, cublasdxDeviceFunctionOption::CUBLASDX_DEVICE_FUNCTION_OPTION_SYMBOL_NAME, sym
    ));
    snprintf(sym, sizeof(sym), "%s_bounds", symbol_prefix);
    CHECK_CUBLASDX(cublasdxSetDeviceFunctionOptionStr(
    ...
    commondxCode code;
    CHECK_CUBLASDX(commondxCreateCode(&code));
    ...
    size_t lto_size = 0;
    CHECK_CUBLASDX(commondxGetCodeLTOIRSize(code, &lto_size));

    std::vector<char> lto(lto_size);
    CHECK_CUBLASDX(commondxGetCodeLTOIR(code, lto.size(), lto.data()));

    if (!write_file(lto.data(), lto.size(), ltoir_output_path, "wb")) {
        res = false;
    }

    CHECK_CUBLASDX(commondxDestroyCode(code));
    CHECK_CUBLASDX(cublasdxDestroyDescriptor(h));
