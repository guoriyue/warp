                ...
                for cu_path in cu_paths:
                    ...
                    if cuda_compiler == "nvcc":
                        if mode == "debug":
                            ...
                            cuda_cmd = f'{nvcc_cmd} --std=c++17 -O3 -ccbin /usr/bin/gcc-12 --compiler-options -fPIC,-fvisibility=hidden {" ".join(_nvcc_opts)} -DNDEBUG -DWP_ENABLE_CUDA=1 -I"{native_dir}" -D{mathdx_enabled} {libmathdx_includes} -o "{cu_out}" -c "{cu_path}"'
                    else:
                        # Use Clang compiler
                        if mode == "debug":
